function catcherror = TalsaRhymeJudge5secUFW
...
T = readtable(fullfile(fileparts(mfilename('fullpath')),'TalsaRhymeJudge5secUFW.csv')); % read stimulus order file
fext = '.wav';
...
instruct = sprintf(['Please judge if the items rhyme or not.\n',...
...
if ~isRecording % if the recording did not start (usually due to an incompatible framerate for the hardware)
    ...
    if ~isRecording % if still not recording, throw an error
        CleanUp;
        error('ffmpeg recording was requested, but failed to start. This may be due to an incompatible framerate')
    end
end
end %startRecording
